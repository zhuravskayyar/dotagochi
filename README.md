...
зберігаються останні сім резервних копій бази. Якщо запуск завершиться помилкою,
вікно термінала залишиться відкритим, а повний лог буде записано у
`~/.local/state/dota-tamagotchi/studio-launch.log`.
